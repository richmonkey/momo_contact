...
+ (NSArray*)getAllRobot {
    ...
    NSString* requestUrl = @"app.json";
    
    NSArray *arrayValue = nil;
    NSInteger statusCode = [MMUapRequest getSync:requestUrl jsonValue:&arrayValue compress:YES];
    if (statusCode != 200 || !arrayValue) {
        
        MLOG(@"获取全部机器人列表失败 statusCode:%d",statusCode);
        return nil;
    }
    ...
    for (NSDictionary* dict in arrayValue) {
        ...
        userInfo.uid            = [[dict objectForKey:@"robot_id"] intValue];
        userInfo.realName       = [dict objectForKey:@"name"];
        userInfo.avatarImageUrl = [dict objectForKey:@"avatar"];
        [[MMMomoUserMgr shareInstance] setUserInfo:userInfo];
        
        DbContactSimple *robotContactInfo = [[[DbContactSimple alloc] init] autorelease];
        robotContactInfo.contactId = [[dict objectForKey:@"robot_id"] intValue];
        robotContactInfo.firstName = [dict objectForKey:@"name"];
        robotContactInfo.avatarUrl = [dict objectForKey:@"avatar"];
        [retArray addObject:robotContactInfo];
    }
...
+(BOOL)changeMobile:(NSString *)mobile password:(NSString *)password withErrorString:(NSString**)errorString {
	...
	[postObject setObject:[NSNumber numberWithInteger:[mobile integerValue]] forKey:@"new_mobile"];
	[postObject setObject:password forKey:@"password"];
	
	NSString* strSource = @"user/change_mobile.json";
	...
	if (lastError != 200) {
		NSLog(@"changed Mobile failed, status code = %d", lastError);
...
+(BOOL)bindingWeibo:(NSDictionary *)weiboDic withErrorString:(NSString**)errorString {
	...
	NSString *strSource = @"bind/create.json";
	NSDictionary *dicRet = [MMUapRequest postSync:strSource withObject:weiboDic];
	
	NSUInteger lastError = [[dicRet valueForKey:@"status"] intValue];
	if (lastError != 200) {
		NSLog(@"bindingWeibo failed, status code = %d", lastError);
        if (dicRet && [dicRet isKindOfClass:[NSDictionary class]]) {
			*errorString = [dicRet objectForKey:@"error"];
			NSLog(@"error:%@",*errorString);
		}
...
+ (BOOL)cancelBindingWeiboWithErrorString:(NSString**)errorString {
	NSString *strSource = @"bind/destroy.json";
	NSMutableDictionary *postObject = [NSMutableDictionary dictionary];
//	[postObject setObject:@"weibo.com" forKey:@"site"];
    [postObject setObject:@"kaixin001.com" forKey:@"site"];
	
	NSDictionary *dicRet = [MMUapRequest postSync:strSource withObject:postObject];
	
	NSUInteger lastError = [[dicRet valueForKey:@"status"] intValue];
	if (lastError != 200) {
		NSLog(@"cancelBindingWeibo failed, status code = %d", lastError);
        if (dicRet && [dicRet isKindOfClass:[NSDictionary class]]) {
			*errorString = [dicRet objectForKey:@"error"];
			NSLog(@"error:%@",*errorString);
		}
