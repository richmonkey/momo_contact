...
@interface MMContactSyncUnitTest : GHTestCase {
	
}
@end

@implementation MMContactSyncUnitTest
- (void)setUp {
	MMLoginService *loginService = [MMLoginService shareInstance];
	
	int result = [loginService doLogin:@"13906932952" password:@"1" zonecode:@"+86"];
...
#if 0
-(void)testSyncCategory {
//  [[MMContactManager instance] killSelf];
//    return;
    
  	MMContactSync *syncer = [[[MMContactSync alloc] init] autorelease];
    
    if (![syncer downloadCategory])
        return;

    if (![syncer uploadCategory])
        return;
    
    if (![syncer syncCategoryMember])
        return;
	...
	if (![syncer downloadCategoryToMomo])
		return;
